...
import { Database, Filter, Download, Radio } from 'lucide-react';
...
export function PipelineDAG({ nodes, failedNode, orientation = 'horizontal', compact = false }: Props) {
      ...
      <div className="relative flex flex-col gap-7 items-center py-8 grid-backdrop rounded-lg border border-dashed border-[#E5E7EB]">
        {nodes.map((node, idx) => (
          <div key={node.id} className="relative flex flex-col items-center">
            <NodeCard node={node} failed={failedNode === node.name} compact={compact} />
            {idx < nodes.length - 1 && (
          ...
          <div key={node.id} className="flex items-center gap-2">
            ...
            {idx < nodes.length - 1 && (
              <div className="flex items-center gap-1">
                <div className="dag-line w-8 h-px" style={{ background: 'none', borderTop: '1px dashed #D1D5DB' }} />
              </div>
            )}
          </div>
        ))}
...
function NodeCard({ node, failed, compact }: { node: DAGNode; failed: boolean; compact: boolean }) {
  return (
    <div
      className={cn(
    ...
    >
      ...
      <h6 className={cn('font-bold text-[#111827] truncate', compact ? 'text-[11px]' : 'text-xs')}>
        {node.name}
      </h6>
      {!compact && node.avg_runtime_sec != null && (
        <p className="text-[9px] text-[#9CA3AF] font-mono mt-1">~{node.avg_runtime_sec}s avg</p>
      )}
    </div>
  );
}
