...
const LOOP_STAGES = [
  { id: 'detected', label: 'Observe', icon: ActivityIcon, statuses: ['Detected'] },
  { id: 'reasoning', label: 'Reason', icon: Brain, statuses: ['Reasoning'] },
  { id: 'planning', label: 'Plan', icon: GitBranch, statuses: ['Planning', 'Awaiting Approval'] },
  { id: 'executing', label: 'Act', icon: Wrench, statuses: ['Executing'] },
  { id: 'evaluating', label: 'Evaluate', icon: CheckCircle2, statuses: ['Evaluating'] },
  { id: 'remediated', label: 'Learn', icon: Zap, statuses: ['Remediated', 'Failed', 'Escalated'] },
];
...
export function IncidentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === 'open')
      return list.filter((i) => i.status !== 'Remediated' && i.status !== 'Escalated');
    if (filter === 'closed')
      return list.filter((i) => i.status === 'Remediated' || i.status === 'Escalated');
    return list;
  }, [state.incidents, filter]);

  const selected = id ? state.incidents.find((i) => i.id === id) : null;
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          {!selected ? (
            ...
            <IncidentDetail
              incident={selected}
              logs={state.logs}
              onApprove={() => approveIncident(selected.id)}
              onReject={() => rejectIncident(selected.id)}
            />
          )}
        </div>
        </main>
...
function IncidentDetail({
  ...
  const [tab, setTab] = useState<'overview' | 'dag' | 'agents' | 'tools' | 'memory'>('overview');
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="px-10 py-7 border-b border-[#E5E7EB] flex items-start justify-between bg-gray-50/30">
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            ...
            <span className="text-[10px] font-mono text-[#9CA3AF]">ID · {incident.id}</span>
      ...
      <div className="flex-1 overflow-y-auto p-10 custom-scrollbar">
        ...
        {tab === 'agents' && (
          <AgentTrace incident={incident} logs={logs} />
        )}
        {tab === 'dag' && pipeline && (
          <div>
...
function Overview({ incident, similar }: { incident: Incident; similar: MemoryEntry[] }) {
  return (
    <div className="space-y-10 max-w-4xl">
      ...
      <section>
        ...
        <div className="bg-[#0F172A] text-emerald-400 p-5 rounded font-mono text-[11px] leading-relaxed shadow-sm overflow-x-auto">
          <span className="text-[#64748B]">[orchestrator] $</span> tail -f pipeline.log
          {'\n'}
          ...
          {incident.proposed_action && (
            <div>
              <h5 className="text-[10px] uppercase font-bold tracking-[0.18em] text-[#9CA3AF] mb-4 flex items-center gap-2">
                ...
                <span className="text-[#111827]">{incident.proposed_action}</span>
              </h5>
                ...
                {similar.map((s) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-[#6B7280] truncate">{s.summary}</p>
                    </div>
...
function MemoryRetrieval({ similar }: { similar: MemoryEntry[] }) {
  if (!similar.length) {
    return (
      <p className="text-[#9CA3AF] italic text-sm">
        Diagnosis hasn't completed yet — RAG matches will appear here once it does.
      </p>
    );
  }
  return (
    <div className="space-y-3 max-w-4xl">
      ...
      {similar.map((s) => (
        <div key={s.id} className="bg-white border border-[#E5E7EB] rounded p-5">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-sm font-semibold">{s.title}</p>
              <p className="text-xs text-[#6B7280] mt-1.5 leading-relaxed">{s.summary}</p>
