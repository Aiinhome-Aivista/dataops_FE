        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-gray-200/60">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-xl font-bold tracking-tight text-[#111827]">Performance Metrics</h1>
              </div>
              <p className="text-xs text-[#6B7280] mt-1">
                Pipelines · RAG retrieval · Mistral LLM latency · auto-refreshes every 15s
              </p>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <StatCard
              ...
              value={ragSummary ? fmtMs(ragSummary.incidents.p95_latency_ms) : '—'}
              icon={Activity}
              accent="cyan"
            ...
            <StatCard
              ...
              value={collections ? collections.incidents + collections.runbooks : 0}
              icon={Database}
              accent="amber"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white border border-[#E5E7EB] rounded-xl shadow-sm">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-[#111827]">Vector retrieval (RAG)</h3>
              </div>
              <div className="p-6 space-y-4">
                {(['incidents', 'runbooks'] as const).map(kind => {
                  const s = ragSummary?.[kind];
                  return (
                    <div key={kind} className="border border-gray-100 rounded-lg p-4 bg-gray-50/50">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                        ...
                        <Metric label="Avg latency" value={fmtMs(s?.avg_latency_ms ?? 0)} />
                        <Metric label="p95 latency" value={fmtMs(s?.p95_latency_ms ?? 0)} />
                        <Metric label="Hit rate" value={s ? fmtPct(s.hit_rate * 100) : '—'} />
                        <Metric label="Top similarity" value={s ? s.avg_top_similarity.toFixed(2) : '—'} />
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="bg-white border border-[#E5E7EB] rounded-xl shadow-sm">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-[#111827]">Mistral LLM calls</h3>
              </div>
              <div className="p-6 space-y-3">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                  <Metric label="Total calls" value={llm?.call_count ?? 0} />
                  <Metric label="Success rate" value={llm ? fmtPct(llm.success_rate * 100) : '—'} />
                  <Metric label="Avg latency" value={fmtMs(llm?.avg_latency_ms ?? 0)} />
                  <Metric label="p95 latency" value={fmtMs(llm?.p95_latency_ms ?? 0)} />
                  <Metric label="Avg prompt size" value={`${llm?.avg_prompt_chars ?? 0} chars`} />
                </div>
                <p className="text-[10px] text-gray-400 leading-relaxed pt-2 border-t border-gray-100">
                  Latency reflects the full Mistral round-trip (prompt assembly + RAG context block + response parsing).
                  Samples capped at the last 500 calls.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-[#E5E7EB] rounded-xl shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-[#111827]">Per-pipeline performance</h3>
            ...
            {loading && sortedRows.length === 0 ? (
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-[#F3F4F6]">
                  {sortedRows.map(r => (
                    <tr key={r.pipeline_id} className="hover:bg-[#F9FAFB] transition-colors">
                      <td className="px-6 py-3 text-xs font-medium text-[#111827] max-w-xs truncate">
                        {r.pipeline_name}
                      </td>
                      <td className="px-3 py-3 text-right text-xs font-mono text-gray-700">{r.runs}</td>
                      <td className="px-3 py-3 text-right text-xs font-mono text-emerald-600">{r.succeeded}</td>
                      <td className="px-3 py-3 text-right text-xs font-mono text-rose-600">{r.failed}</td>
                      ...
                      <td className="px-3 py-3 text-right text-xs font-mono text-gray-700">{fmtSec(r.avg_duration_sec)}</td>
                      <td className="px-3 py-3 text-right text-xs font-mono text-gray-700">{fmtSec(r.p50_duration_sec)}</td>
                      <td className="px-3 py-3 text-right text-xs font-mono text-gray-700">{fmtSec(r.p95_duration_sec)}</td>
                      <td className="px-3 py-3 text-right text-xs font-mono text-gray-700">{fmtSec(r.p99_duration_sec)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
